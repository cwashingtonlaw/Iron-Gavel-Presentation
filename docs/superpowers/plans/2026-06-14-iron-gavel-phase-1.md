...
The Xcode project is generated by **XcodeGen** from `project.yml` (see Task 1). This changes how three things that the per-task steps below mention are actually done — read this once, then ignore those instructions in each task:

1. **"Add file to target."** Target membership is decided by the source-glob patterns in `project.yml`. Every file you create under `IronGavel/`, `IronGavelTests/`, or `IronGavelUITests/` is automatically a member of the corresponding target. After creating files, just run `xcodegen generate` to refresh the `.xcodeproj`. Ignore any "drag into Xcode" or "File Inspector → Target Membership" instructions.

2. **"Add to Copy Bundle Resources."** XcodeGen's source globs include resource files (JSON, plists, images) automatically. Files under `IronGavelTests/Fixtures/` are bundled into the test bundle; files under `IronGavel/Resources/` are bundled into the app. No manual Build Phase edits.

3. **Build / test commands.** Per-task `⌘B` means:

   ```bash
   xcodegen generate && \
   xcodebuild -project IronGavel.xcodeproj -scheme IronGavel \
     -destination 'platform=iOS Simulator,name=iPad (10th generation)' build | tail -20
   ```
   ...
   xcodegen generate && \
   xcodebuild -project IronGavel.xcodeproj -scheme IronGavel \
     -destination 'platform=iOS Simulator,name=iPad (10th generation)' test | tail -40
   ```

   If "iPad (10th generation)" is not available, pick another iPad simulator from `xcrun simctl list devices available | grep iPad`. Always run `xcodegen generate` first after creating new files so the project picks them up.
...
**Approach note.** The project is generated from `project.yml` by `xcodegen`, not created via the Xcode GUI. The generated `.xcodeproj` is gitignored; every clone runs `xcodegen generate`. This lets headless agents and humans both bootstrap the project identically and removes "drag to target" steps from every later task — target membership is decided by the source-glob patterns in `project.yml`.

**Files:**
- Create: `project.yml` (XcodeGen spec)
- Create: `IronGavel/App/IronGavelApp.swift` (stub)
- Create: `IronGavel/Resources/Info.plist`
- Create: `IronGavelTests/Placeholder.swift`
- Create: `IronGavelUITests/Placeholder.swift`
- Create: `.gitignore`
...
Expected: version prints (e.g. `Version: 2.40.1`).
...
name: IronGavel
options:
  bundleIdPrefix: com.danielswashington
  deploymentTarget:
    iOS: "17.0"
...
targets:
  IronGavel:
    ...
    sources:
      - path: IronGavel
    info:
      path: IronGavel/Resources/Info.plist
      properties:
        UILaunchScreen:
          UIColorName: ""
        UIApplicationSceneManifest:
          ...
          UISceneConfigurations:
            UIWindowSceneSessionRoleExternalDisplayNonInteractive:
              - UISceneConfigurationName: Jury
                UISceneDelegateClassName: $(PRODUCT_MODULE_NAME).JurySceneDelegate
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: com.danielswashington.irongavel
  ...
  IronGavelTests:
    ...
    settings:
      base:
        ...
        TEST_HOST: $(BUILT_PRODUCTS_DIR)/IronGavel.app/IronGavel
  IronGavelUITests:
    type: bundle.ui-testing
    platform: iOS
    sources:
      - path: IronGavelUITests
    dependencies:
      - target: IronGavel
schemes:
  IronGavel:
...
- [ ] **Step 3: Write the placeholder Swift sources**
...
(XcodeGen merges the scene manifest from `project.yml` into this file at generation time.)
...
# Carthage / Pods (defensive)
Pods/
Carthage/Build/
```

Note: `*.xcodeproj/` is ignored — the project is always regenerated from `project.yml`.
...
Expected: `Created project at IronGavel.xcodeproj`.
...
Expected: `** BUILD SUCCEEDED **`. If the destination name fails, list available simulators with `xcrun simctl list devices available | grep iPad` and use one of those names.

- [ ] **Step 7: Commit**

```bash
git add project.yml .gitignore IronGavel IronGavelTests IronGavelUITests
git commit -m "chore: scaffold IronGavel iPad SwiftUI app via XcodeGen"
```
...
XcodeGen includes files under `IronGavelTests/Fixtures/` automatically. After creating the fixture, run `xcodegen generate` to refresh the project.
...
Both files sit under `IronGavelTests/Fixtures/BadVersion/` and `IronGavelTests/Fixtures/BadJSON/`. To preserve the subdirectory structure inside the test bundle, add this to the `IronGavelTests` target in `project.yml` (under `sources`):
...
Then run `xcodegen generate`. The `type: folder` directive creates folder references so the subdirectory survives inside the bundle and `Bundle.url(forResource:)` finds them by folder name.
...
- [ ] **Step 3: Confirm Info.plist scene manifest**

`project.yml` already declares the external-scene manifest (set in Task 1). Re-run `xcodegen generate` and verify `IronGavel/Resources/Info.plist` contains the `UIApplicationSceneManifest` block with the `Jury` configuration. If it does not, your `project.yml` is out of sync — fix `project.yml` rather than editing the plist directly.
...
Copy `IronGavelTests/Fixtures/Trial/exhibits.json` to `IronGavel/Resources/ui-test-exhibits.json`. XcodeGen includes it in the app bundle automatically via the `IronGavel` source glob; run `xcodegen generate` to refresh.
